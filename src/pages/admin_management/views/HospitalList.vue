<template>
  <v-container fill-height class="prescription-list__container w-100">
    ...
    <v-dialog v-model="showDetail" width="1200">
      <hospital-info :hospital-info="selectedHospital" @confirmedUpdateInfo="confirmedUpdateHospitalInfo"/>
    </v-dialog>

    <v-dialog v-model="showDialogCreateAccount" width="800">
      <create-account @onSubmit="submitedCreateHospitalAccount"/>
    </v-dialog>
...
export default {
  ...
  data() {
    return {
      ...
      isDialog: true,
      selectedHospital: {},
      headersTableHospital: [
        {text: this.$t('id'), value: 'id', width: '6%'},
        {text: this.$t('hospital'), value: 'name', width: '15%'},
  ...
  methods: {
    ...
    submitedCreateHospitalAccount(data) {
      console.log(data);
      this.isLoading += 1;
      let vm = this;
      adminService.createHospitalAccount(data)
          .then(response => {
            if (response.status === 200) {
              vm.getDataFromApi();
              vm.showSnackbarFunc(vm.$t('success'), 'success');
    ...
    confirmedUpdateHospitalInfo(inputData) {
      let vm = this;
      this.isLoading += 1;
      ...
      adminService.updateHospitalInfo(this.selectedHospital.id, data)
          .then(response => {
            if (response.status === 200) {
              vm.showDetail = false;
              vm.getDataFromApi();
    ...
    getDataFromApi() {
      ...
      adminService.getHospitalList(page, itemsPerPage)
          .then(response => {
  ...
  components: {MySnackbar, CreateAccount, HospitalInfo},
  computed: {
    isLoading: {
      get() {

<template>
  <v-container fill-height class="prescription-list__container w-100">
    ...
    <v-dialog v-model="showDetail" width="1200">
      <doctor-info :doctor-info="selectedDoctor" @confirmedUpdateInfo="confirmedUpdateDoctorInfo"/>
    </v-dialog>

    <v-dialog v-model="showDialogCreateAccount" width="800">
      <create-account @onSubmit="submitedCreateDoctorAccount"/>
    </v-dialog>
...
export default {
  ...
  data() {
    return {
      ...
      headersTableDoctor: [
        ...
        {text: this.$t('doctorName'), value: 'name', width: '15%'},
  ...
  methods: {
    ...
    submitedCreateDoctorAccount(data) {
      console.log(data);
      this.isLoading += 1;
      let vm = this;
      adminService.createDoctorAccount(data)
          .then(response => {
            if (response.status === 200) {
              vm.getDataFromApi();
              vm.showSnackbarFunc(vm.$t('success'), 'success');
    ...
    confirmedUpdateDoctorInfo(data) {
      let vm = this;
      this.isLoading += 1;
      console.log(data);
      adminService.updateDoctorInfo(this.selectedDoctor.id, data)
          .then(response => {
            if (response.status === 200) {
              vm.showDetail = false;
              vm.getDataFromApi();
    ...
    getDataFromApi() {
      ...
      adminService.getDoctorList(page, itemsPerPage)
          .then(response => {
  ...
  components: {MySnackbar, CreateAccount, DoctorInfo},
  computed: {
    isLoading: {
      get() {

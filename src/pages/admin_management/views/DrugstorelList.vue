<template>
  <v-container fill-height class="prescription-list__container w-100">
    ...
    <v-dialog v-model="showDetail" width="1200">
      <drugstore-info :drugstore-info="selectedDrugstore" @confirmedUpdateInfo="confirmedUpdateDrugstoreInfo"/>
    </v-dialog>

    <v-dialog v-model="showDialogCreateAccount" width="800">
      <create-account @onSubmit="submitedCreateDrugstoreAccount"/>
    </v-dialog>
...
import {adminService} from "@/pages/admin_management/services/admin.services";
import DrugstoreInfo from "@/components/DrugstoreInfo";
import CreateAccount from "@/components/CreateAccount";
import MySnackbar from "@/components/MySnackbar";

export default {
  name: 'DrugstoreListView',
  data() {
    return {
      showSnackbar: false,
      snackBarText: "",
      ...
      isDialog: true,
      selectedDrugstore: {},
      headersTableDrugstore: [
        {text: this.$t('id'), value: 'id', width: '6%'},
        {text: this.$t('drugstore'), value: 'name', width: '15%'},
        {text: this.$t('phoneNumber'), value: 'user.phone_number'},
        {text: this.$t('departmentAdmin'), value: 'department'},
  ...
  methods: {
    activeUser(item) {
      ...
      let vm = this;
      this.isLoading += 1;
      adminService.activeUser(item.user.id)
          .then(response => {
    ...
    submitedCreateDrugstoreAccount(data) {
      console.log(data);
      this.isLoading += 1;
      let vm = this;
      adminService.createDrugstoreAccount(data)
          .then(response => {
            if (response.status === 200) {
              vm.getDataFromApi();
              vm.showSnackbarFunc(vm.$t('success'), 'success');
    ...
    confirmedUpdateDrugstoreInfo(inputData) {
      let vm = this;
      this.isLoading += 1;
      ...
      adminService.updateDrugstoreInfo(this.selectedDrugstore.id, data)
          .then(response => {
            if (response.status === 200) {
              vm.showDetail = false;
              vm.getDataFromApi();
    ...
    getDataFromApi() {
      ...
      adminService.getDrugstoreList(page, itemsPerPage)
          .then(response => {
    ...
    selectPrescription(item) {
      this.selectedDrugstore = item;
      this.showDetail = true;
    },
  },
  components: {MySnackbar, CreateAccount, DrugstoreInfo},
  computed: {
    isLoading: {
      get() {

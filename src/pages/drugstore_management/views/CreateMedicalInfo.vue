...
export default {
  ...
  methods: {
    ...
    getDataFromApi() {
      ...
      drugstoreService.getMedicineList(page, itemsPerPage, this.searchMedicine)
          .then(response => {
    ...
    checkPatientPhone() {
      if (this.patientPhoneNumber.length > 0) {
        this.isLoading += 1;
        drugstoreService.getPatientInfo(this.patientPhoneNumber)
            .then(response => {
    ...
    confirmedDialog(value) {
      ...
      this.isLoading += 1;
      drugstoreService.createMedicalInfo(this.patientInfo.id, this.patientInfo.medical_info)
          .then(response => {
            if (response.status === 200) {
              console.log("success");
              this.showSnackbarFunc(this.$t('success'), 'success');

...
export default {
  ...
  methods: {
    ...
    getDataFromApi() {
      ...
      hospitalService.getMedicineList(page, itemsPerPage, this.searchMedicine)
          .then(response => {
    ...
    checkPatientPhone() {
      if (this.patientPhoneNumber.length > 0) {
        this.isLoading += 1;
        hospitalService.getPatientInfo(this.patientPhoneNumber)
            .then(response => {
    ...
    confirmedDialog(value) {
      ...
      this.isLoading += 1;
      hospitalService.createMedicalInfo(this.patientInfo.id, this.patientInfo.medical_info)
          .then(response => {
            if (response.status === 200) {
              console.log("success");
              this.showSnackbarFunc(this.$t('success'), 'success');

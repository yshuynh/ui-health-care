...
export default {
  ...
  data() {
    return {
      ...
      headersTableDoctor: [
        ...
        {text: this.$t('departmentHospital'), value: 'department'},
  ...
  methods: {
    activeUser(item) {
      ...
      let vm = this;
      this.isLoading += 1;
      hospitalService.activeUser(item.user.id)
          .then(response => {
    ...
    submitedCreateDoctorAccount(data) {
      ...
      this.isLoading += 1;
      let vm = this;
      hospitalService.createDoctorAccount(data)
          .then(response => {
            if (response.status === 200) {
              vm.getDataFromApi();
              vm.showSnackbarFunc(vm.$t('success'), 'success');
    ...
    confirmedUpdateDoctorInfo(data) {
      ...
      this.isLoading += 1;
      console.log(data);
      hospitalService.updateDoctorInfo(this.selectedDoctor.id, data)
          .then(response => {
            if (response.status === 200) {
              vm.showDetail = false;
              vm.getDataFromApi();
    ...
    getDataFromApi() {
      ...
      hospitalService.getDoctorList(page, itemsPerPage)
          .then(response => {
